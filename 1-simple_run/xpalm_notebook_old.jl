...
meteo = let
    m = CSV.read("../2-sites_comparison/0-data/Meteo_predictions_all_sites_cleaned.csv", DataFrame)
    m.duration .= Dates.Day(1)
    transform!(groupby(m, :Site), :date => (x -> 1:length(x)) => :timestep)
    filter!(x -> x.Site == "SMSE", m)
    Weather(m)
    m
end
...
let
    ...
    for (scale, df) in dfs
        ...
        save("simulation_results_$scale.png", draw(plt; figure=(; size=(800, 300)), facet=(; linkxaxes=:none, linkyaxes=:none), legend=(; show=false)))
