...
function model_mapping_theft(p)
    Dict(
        ...
        "Leaf" => (
            ...
            MultiScaleModel(
                model=RankLeafPruningWindow(
                    rank=p.parameters[:management][:rank_leaf_pruning],
                    rank_window=p.parameters[:management][:manual_pruning][:rank],
                    start_date=p.parameters[:management][:manual_pruning][:start_date],
                    duration=p.parameters[:management][:manual_pruning][:duration],
                ),
