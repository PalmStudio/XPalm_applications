...
includet("1-code/mapping.jl")

#! Number of months after planting for the first time-step:
MAP_0 = -18 # It is negative because we start from the seed

c = XPalm.PlantMeteo.Constants()
meteo_raw = CSV.read("0-data/Meteo_predictions_all_sites.csv", DataFrame)
select!(
    meteo_raw,
    :Site,
...
meteo_cleaned_gp = groupby(meteo_cleaned, :Site)
meteos = [Weather(i) for i in meteo_cleaned_gp]

# df_plot = transform(meteo_cleaned_gp, :Site => :site, :Precipitations => cumsum => :cumulative_precipitations)
...
begin
    ...
    # params_SMSE[:field_capacity] = 0.25
    # params_SMSE[:wilting_point_1] = 0.12
    # params_SMSE[:wilting_point_2] = 0.12

    params_PRESCO = copy(params_default)
    params_PRESCO[:latitude] = 6.137
    params_PRESCO[:altitude] = 15.5
    # params_PRESCO[:field_capacity] = 0.25
    # params_PRESCO[:wilting_point_1] = 0.12
    # params_PRESCO[:wilting_point_2] = 0.12

    params_TOWE = copy(params_default)
    # params_TOWE[:latitude] = 7.65
    params_TOWE[:latitude] = 7.00
    params_TOWE[:altitude] = 15.5
    # params_TOWE[:field_capacity] = 0.25
    # params_TOWE[:wilting_point_1] = 0.12
    # params_TOWE[:wilting_point_2] = 0.12
    ...
    # mapping_xpalm = xpalm_mapping(palm)
    # ex = XPalm.PlantSimEngine.SequentialEx()
    # sim = XPalm.PlantSimEngine.run!(palm.mtg, mapping_xpalm, m, outputs=out_vars, executor=ex, check=false)
    # #!######
    ...
    dfs_all = leftjoin(dfs_all, meteo_cleaned, on=[:Site, :timestep,])
...
scatter(dfs_plant.timestep[1:end-1], dfs_plant.phytomer_count[2:end] .- dfs_plant.phytomer_count[1:end-1], color=:green, markersize=3)
